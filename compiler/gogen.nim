...
# proc genjs(p: PProc, n: PNode, r: var TCompRes)
...
proc myProcess(b: PPassContext, n: PNode): PNode =
  ...
  genModule(p, n)
  # echo p.g.code
  echo "#2198, add p.locals"
  add(p.g.code, p.locals)
  echo "#2200, add p.body"
  add(p.g.code, p.body)
...
const GogenPass* = makePass(myOpen, myOpenCached, myProcess, myClose)
